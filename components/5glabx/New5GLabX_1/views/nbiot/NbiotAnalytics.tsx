...
interface NbiotAnalyticsProps {
  executionId?: string | null;
}

const NbiotAnalytics: React.FC<NbiotAnalyticsProps> = ({ executionId }) => {
  ...
  useEffect(() => {
    const handleTestExecution = (event: any) => {
      if (event.type === '5GLABX_TEST_EXECUTION' && event.detail?.testCaseData) {
        console.log('🔥 NbiotAnalytics: Received test execution event:', event.detail);
      ...
      <div className="bg-white border-b border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-xl font-bold text-gray-900">Nbiot Analytics</h2>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mb-2">
            {isReceivingData ? '🎯 Nbiot Analytics - 3GPP standard parameters...' : '📭 No nbiot analytics data available'}
          </p>
          <p className="text-gray-400 text-xs">
            {isReceivingData
              ? 'Real-time analysis with 3GPP standard parameters'
              : 'Start a test execution to see nbiot analytics data'}
          </p>
        </div>
      </div>
    </div>
