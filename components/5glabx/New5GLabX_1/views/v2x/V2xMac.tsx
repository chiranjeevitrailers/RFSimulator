...
interface V2xMacProps {
  executionId?: string | null;
}

const V2xMac: React.FC<V2xMacProps> = ({ executionId }) => {
  ...
  useEffect(() => {
    const handleTestExecution = (event: any) => {
      if (event.type === '5GLABX_TEST_EXECUTION' && event.detail?.testCaseData) {
        console.log('🔥 V2xMac: Received test execution event:', event.detail);
      ...
      <div className="bg-white border-b border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-xl font-bold text-gray-900">V2x Mac</h2>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mb-2">
            {isReceivingData ? '🎯 V2x Mac - 3GPP standard parameters...' : '📭 No v2x mac data available'}
          </p>
          <p className="text-gray-400 text-xs">
            {isReceivingData
              ? 'Real-time analysis with 3GPP standard parameters'
              : 'Start a test execution to see v2x mac data'}
          </p>
        </div>
      </div>
    </div>
