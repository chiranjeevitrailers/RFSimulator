...
import { IMSSIPTestCaseGenerator } from '@/lib/ims-sip-test-generator';
import { 
  Play, 
  Pause, 
  Square, 
...
interface IMSSIPTestCaseViewerProps {
  userId: string;
}

const IMSSIPTestCaseViewer: React.FC<IMSSIPTestCaseViewerProps> = ({ userId }) => {
  ...
  const generateTestCases = async () => {
    try {
      setIsLoading(true);
      const templates = IMSSIPTestCaseGenerator.generateAllIMSSIPTestCases();
      const testCases = templates.map(template => IMSSIPTestCaseGenerator.convertToTestCase(template));
  ...
  const getTestTypeIcon = (testCase: TestCase) => {
    ...
    } else if (name.includes('call') || name.includes('invite')) {
      return <Phone className="w-4 h-4 text-green-500" />;
    } else if (name.includes('presence') || name.includes('publish') || name.includes('subscribe')) {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading IMS/SIP test cases...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">IMS/SIP Test Cases</h2>
              <p className="text-sm text-gray-500">Comprehensive IMS and SIP protocol test cases</p>
            </div>
          </div>
      ...
      {filteredTestCases.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            {testCases.length === 0 
              ? "No IMS/SIP test cases have been generated yet."
...
export default IMSSIPTestCaseViewer;