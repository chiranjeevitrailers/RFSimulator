...
import { NBIoTV2XNTNTestCaseGenerator } from '@/lib/nbiot-v2x-ntn-test-generator';
import { 
  Play, 
  Pause, 
  Square, 
...
interface NBIoTV2XNTNTestCaseViewerProps {
  userId: string;
}

const NBIoTV2XNTNTestCaseViewer: React.FC<NBIoTV2XNTNTestCaseViewerProps> = ({ userId }) => {
  ...
  const loadTestCases = async () => {
    try {
      ...
      const data = await TestCaseService.getTestCases({
        protocol_version: ['NB-IoT', 'V2X', 'NTN'],
        limit: 200
      });
      setTestCases(data);
    } catch (error) {
  ...
  const generateTestCases = async () => {
    try {
      setIsLoading(true);
      const templates = NBIoTV2XNTNTestCaseGenerator.generateAllNBIoTV2XNTNTestCases();
      const testCases = templates.map(template => NBIoTV2XNTNTestCaseGenerator.convertToTestCase(template));
  ...
  const getTestTypeIcon = (testCase: TestCase) => {
    const name = testCase.name.toLowerCase();
    const category = testCase.category;
    
    if (category === 'NB_IoT') {
      if (name.includes('rrc')) return <Network className="w-4 h-4 text-blue-500" />;
      if (name.includes('data')) return <Database className="w-4 h-4 text-indigo-500" />;
      if (name.includes('handover')) return <Switch className="w-4 h-4 text-cyan-500" />;
      if (name.includes('power')) return <Zap className="w-4 h-4 text-yellow-500" />;
      return <Wifi className="w-4 h-4 text-blue-500" />;
    } else if (category === 'V2X') {
      if (name.includes('pc5')) return <Network className="w-4 h-4 text-green-500" />;
      if (name.includes('bsm') || name.includes('cam') || name.includes('denm')) return <Signal className="w-4 h-4 text-emerald-500" />;
      if (name.includes('v2v') || name.includes('v2i') || name.includes('v2p')) return <Car className="w-4 h-4 text-teal-500" />;
      if (name.includes('security')) return <Shield className="w-4 h-4 text-red-500" />;
      return <Car className="w-4 h-4 text-green-500" />;
    } else if (category === 'NTN') {
      if (name.includes('satellite')) return <Satellite className="w-4 h-4 text-purple-500" />;
      if (name.includes('handover')) return <Switch className="w-4 h-4 text-violet-500" />;
      if (name.includes('beam')) return <Target className="w-4 h-4 text-pink-500" />;
      if (name.includes('power')) return <Zap className="w-4 h-4 text-orange-500" />;
      return <Satellite className="w-4 h-4 text-purple-500" />;
    }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading NB-IoT, V2X, NTN test cases...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">NB-IoT, V2X, NTN Test Cases</h2>
              <p className="text-sm text-gray-500">Comprehensive IoT, Vehicle, and Satellite protocol test cases</p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg flex items-center justify-center">
                  {getTestTypeIcon(selectedTestCase)}
                </div>
      ...
      {filteredTestCases.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            {testCases.length === 0 
              ? "No NB-IoT, V2X, NTN test cases have been generated yet."
...
export default NBIoTV2XNTNTestCaseViewer;