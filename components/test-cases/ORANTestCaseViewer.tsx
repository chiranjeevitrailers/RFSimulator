...
import { ORANTestCaseGenerator } from '@/lib/oran-test-generator';
import { 
  Play, 
  Pause, 
  Square, 
...
interface ORANTestCaseViewerProps {
  userId: string;
}

const ORANTestCaseViewer: React.FC<ORANTestCaseViewerProps> = ({ userId }) => {
  ...
  const generateTestCases = async () => {
    try {
      setIsLoading(true);
      const templates = ORANTestCaseGenerator.generateAllORANTestCases();
      const testCases = templates.map(template => ORANTestCaseGenerator.convertToTestCase(template));
  ...
  const getTestTypeIcon = (testCase: TestCase) => {
    ...
    } else if (name.includes('o1')) {
      return <Database className="w-4 h-4 text-purple-500" />;
    } else if (name.includes('x2') || name.includes('xn')) {
      return <Router className="w-4 h-4 text-orange-500" />;
    } else if (name.includes('handover')) {
      return <Switch className="w-4 h-4 text-indigo-500" />;
    } else if (name.includes('policy')) {
      return <Shield className="w-4 h-4 text-red-500" />;
    } else if (name.includes('configuration')) {
      return <Settings className="w-4 h-4 text-gray-500" />;
    } else if (name.includes('performance')) {
      return <BarChart className="w-4 h-4 text-teal-500" />;
    } else {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading O-RAN test cases...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">O-RAN Test Cases</h2>
              <p className="text-sm text-gray-500">Comprehensive Open RAN protocol test cases</p>
            </div>
          </div>
      ...
      {filteredTestCases.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            {testCases.length === 0 
              ? "No O-RAN test cases have been generated yet."
...
export default ORANTestCaseViewer;