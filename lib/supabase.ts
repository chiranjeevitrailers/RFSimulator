...
export const createClient = () => {
  ...
  _supabaseClient = createSbClient(url, key)
  return _supabaseClient
}
...
export const db = {
  ...
  async createTestCase(testCaseData: Partial<TestCase>) {
    checkSupabase()
    const { data, error } = await supabase!.from("test_cases").insert([testCaseData]).select().single()
  ...
  async createTestExecution(executionData: Partial<TestExecution>) {
    checkSupabase()
    const { data, error } = await supabase!.from("test_executions").insert([executionData]).select().single()
