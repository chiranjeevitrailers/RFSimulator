-- 5GLabX Platform - Complete Database Setup (Fixed Version)
-- This file contains all essential SQL for setting up the 5GLabX Platform database
-- Handles existing objects gracefully with IF NOT EXISTS and DROP IF EXISTS
...
-- ==============================================
-- 5. FUNCTIONS AND TRIGGERS (Fixed)
-- ==============================================
...
DROP TRIGGER IF EXISTS update_test_case_executions_updated_at ON test_case_executions;
...
DROP POLICY IF EXISTS "Admins can manage all test executions" ON test_case_executions;
CREATE POLICY "Admins can manage all test executions" ON test_case_executions 
  FOR ALL TO service_role USING (true);

-- Test results RLS Policies
DROP POLICY IF EXISTS "Users can view results of their executions" ON test_case_results;
...
DROP POLICY IF EXISTS "Users can insert results for their executions" ON test_case_results;
...
DROP POLICY IF EXISTS "Admins can manage all test results" ON test_case_results;
...
BEGIN
    ...
    RAISE NOTICE '⚡ Fixed version handles existing objects gracefully';
END $$;